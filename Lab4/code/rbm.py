...
class RestrictedBoltzmannMachine():
    ...
    def cd1(self, visible_trainset, n_iterations=10000):
        ...
        for it in range(n_iterations + 1):  # +1 for printing with %-operations
            ...
            p_h_v_1, h_1 = self.get_h_given_v(p_v_h_1)  # final update of hidden units should use probs
            ...
            if it % self.rf["period"] == 0 and self.is_bottom:
                viz_rf(weights=self.weight_vh[:, self.rf["ids"]].reshape((self.image_size[0], self.image_size[1], -1)),
                       it=it, grid=self.rf["grid"], name_add="")
    ...
    def get_v_given_h(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            labels = softmax(support[:, -self.n_labels:])
            support = sigmoid(support[:, :-self.n_labels])

            p_v_h = np.concatenate((support, labels), axis=1)
